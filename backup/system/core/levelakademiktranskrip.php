<?php
function deflevelakademiktranskrip(){
buka("KAPRODI :: TRAnskrip Nilai");  
$tAktif	=TahunAktif();
$tahun 		= (empty($_POST[tahun]))? $tAktif : $_POST[tahun];
$nim 		= (empty($_POST[NIM]))? '' : $_POST[NIM];
echo "<form method=post action='go-levelakademiktranskrip.html'>
...
echo"<legend>TRANSKRIP NILAI
	...
	<a class='btn btn-inverse' href='cetaktrans-Transkrip-$nim-$data[kode_jurusan].html' target='_blank'>Cetak</a>
	</div></legend>";
...
$sql="SELECT * FROM matakuliah WHERE Identitas_ID='$_SESSION[Identitas]' AND Jurusan_ID='$data[kode_jurusan]' AND Kurikulum_ID IN ('41',$data[Kurikulum_ID]) ORDER BY Kode_mtk";
  ...
  $no++;
  $sqlr="SELECT * FROM krs WHERE NIM='$nim' AND Jadwal_ID='$data[Kode_mtk]'";
