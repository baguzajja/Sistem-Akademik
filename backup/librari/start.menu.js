...
with (this){
 for (mN in menu){
  ...
  with (menu[mN][0]){
   ...
   if (pM && typeof(offX)=='number') newX = Math.max(sX,Math.min(newX+pM[0].lyr.x()+pI.x(), wX-menuW-(isIE?5:20)));
   if (pM && typeof(offY)=='number') newY = Math.max(sY,Math.min(newY+pM[0].lyr.y()+pI.y(), wY-menuH-(isIE?5:20)));
   lyr.x(newX);
   lyr.y(newY);
  }
 }
...
function popAddItem() { with (this) with (actMenu[0]) {
 ...
 if (aI.outBorder){if (isNS4) aI.pad++;}
...
var hBar = new ItemStyle(80, 0, '', 0, 0, '', 'highlight', 'lowText', 'highText', 
  'rootBorder', 'rootBorder', null, null, 'hand', 'default');
var subM = new ItemStyle(22, 0, '&gt;', 0, 3, 'white', 'highlight', 'lowText', 'highText',
