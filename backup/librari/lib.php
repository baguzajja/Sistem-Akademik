...
$entry_line = "$nama | $dtime | $ip | $agent? | $uri | $ref | $asli | $via | $aksi \n"; 
...
function PesanOk2($jdl, $isi='', $link1, $link2) {
echo "<div class='panel-header'><i class='icon-sign-blank'></i> $jdl </div>
  <div class='panel-content'>
		...
		<center>
		<div class='btn-group'>
		<a class='btn btn-success iframe' href='$link1'><i class='icon-ok'></i> OK </a><a class='btn btn-Inverse' href='$link2' target='_Blank'><i class='icon-print'></i> Cetak </a></center>
		</div>
  </div>";
}
function PesanOk3($jdl, $isi='', $link1, $link2) {
echo "<div class='panel-header'><i class='icon-sign-blank'></i> $jdl </div>
  <div class='panel-content'>
		<div class='alert alert-success'>
			<button class='close' data-dismiss='alert'>&times;</button>
		...
		<center>
		<div class='btn-group'>
		<a class='btn btn-success' href='$link1'><i class='icon-ok'></i> OK </a><a class='btn btn-Inverse' href='$link2'><i class='icon-download-alt '></i> Download BackUp </a>
		</center>
		</div>
  </div>";
}
function PesanOk($jdl, $isi='', $link) {
echo "<div class='panel-header'><i class='icon-sign-blank'></i> $jdl </div>
  <div class='panel-content'>
		<div class='alert alert-success'>
			<button class='close' data-dismiss='alert'>&times;</button>
		...
		<center><a class='btn btn-success' href='$link'><i class='icon-ok'></i> OK </a></center>
  </div>";
}
function ErrorMsg($jdl='Error', $isi='') {
  echo "<div class='alert alert-error'>
			<button class='close' data-dismiss='alert'>&times;</button>
			<strong>$jdl !</strong> $isi
		</div>";
}
function InfoMsg($jdl='Info', $isi='') {
  echo "<div class='alert alert-info'>
			<button class='close' data-dismiss='alert'>&times;</button>
			<strong>$jdl !</strong> $isi.
		</div>";
}
function SuksesMsg($jdl='Sukses', $isi='') {
  echo "<div class='alert alert-success'>
			<button class='close' data-dismiss='alert'>&times;</button>
			<strong>$jdl !</strong> $isi.
		</div>";
}
function ErrorMsgs($jdl='Error', $isi='') {
  echo "<div class='panel-content'><div class='alert alert-error'>
			<button class='close' data-dismiss='alert'>&times;</button>
			<strong>$jdl !</strong> $isi
		</div></div>";
}
function InfoMsgs($jdl='Info', $isi='') {
  echo "<div class='alert alert-info'>
			<button class='close' data-dismiss='alert'>&times;</button>
			<strong>$jdl !</strong> $isi.
		</div>";
}
function SuksesMsgs($jdl='Sukses', $isi='') {
  echo "<div class='panel-content'><div class='alert alert-success'>
			<button class='close' data-dismiss='alert'>&times;</button>
			<strong>$jdl !</strong> $isi.
		</div></div>";
}
...
function GetName($table,$where,$id,$field){
	...
	if ($id!='')
	{
		$cek=mysql_query("SELECT * FROM $table WHERE $where='$id' LIMIT 0,1");
		if (mysql_num_rows($cek)>0)
		{
			$row=mysql_fetch_array($cek);
			$Name=$row[$field];
...
function NamaKelas($id){
	...
	if ($id!='')
	{
		$cek=mysql_query("SELECT * FROM program WHERE Program_ID='$id' LIMIT 0,1");
		if (mysql_num_rows($cek)>0)
		{
			$row=mysql_fetch_array($cek);
...
function TotalBayar($id){
	...
	if ($id!='')
	{
		$cek=mysql_query("SELECT SUM(Debet) AS total FROM transaksi WHERE AccID='$id' AND Buku IN ('1','2','8','9') AND Transaksi='17' LIMIT 0,1");
		if (mysql_num_rows($cek)>0)
		{
			$row=mysql_fetch_array($cek);
...
function get_Pt($id){
	...
	if ($id!='')
	{
		$cek=mysql_query("SELECT * FROM perguruantinggi WHERE PerguruanTinggiID='$id' LIMIT 0,1");
		if (mysql_num_rows($cek)>0)
		{
			$row=mysql_fetch_array($cek);
			$Pt=$row['Nama'];
