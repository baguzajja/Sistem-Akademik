...
class SimpleCalendar {
	...
	public function show( $echo = true ) {
		...
		$namaBulan=strtoupper(getBulan($this->now['mon']));
		$out = '<table class="month"><thead>
				<tr><th colspan="7">'.$namaBulan.' - '.$this->now['year'].'</th></tr>';
		$out .= '<tr class="days"><td>Se</td><td>Se</td><td>Ra</td><td>Ka</td><td>Ju</td><td>Sa</td><td class="minggu">Mi</td></tr></thead>';
		...
		for( $i = 1; $i <= $no_days; $i++ ) {
			...
			if($i == $this->now['mday'] && $this->now['mon'] == date('n')) {
				$out .= '<td class="minggu">'.$i.'</td>';
			} elseif( is_array($dHtml_arr) ) {
				foreach( $dHtml_arr as $eid => $dHtml ) {
					$dr .= $dHtml;
				}
				$out .= '<td bgcolor="'.$dHtml.'" class="pilih">'.$i.'</td>';
			} else {
				$out .= '<td>'.$i.'</td>';
			}
		...
		$out .= ($count != 1 ? str_repeat('<td>&nbsp;</td>', 8 - $count) : '') . "</table>\n";
		if( $echo ) {
			echo $out;
		}
		return $out;
	}
public function showPdf( $echo = true ) {
		if( $this->wday_names ) {
			$wdays = $this->wday_names;
		} else {
			$today = (86400 * (date("N")));
		...
		$out = '<table class="month" style="width: 100%;">
				<tr><th colspan="7">'.$namaBulan.' - '.$this->now['year'].'</th></tr>';
		$out .= '<tr class="days">
		<td style="width: 12%;">Se</td>
		<td style="width: 12%;">Se</td>
		<td style="width: 12%;">Ra</td>
		<td style="width: 12%;">Ka</td>
		<td style="width: 12%;">Ju</td>
		<td style="width: 12%;">Sa</td>
		<td class="minggu" style="width: 16%;">Mi</td></tr>';
		...
		$out .= ($count != 1 ? str_repeat('<td>&nbsp;</td>', 8 - $count) : '') . "</tr></table>";
		if( $echo ) {
			echo $out;
		}
		return $out;
